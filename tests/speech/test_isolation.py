...
class TestNoRobotDependencyAtRuntime:
    def test_app_robot_not_imported_as_a_side_effect(self):
        ...
        # pulled in as a transitive import. The removed modules are
        # restored afterwards (even on failure) so this check does not
        # leak global interpreter state into tests that run later in
        # the same session (e.g. other suites that expect app.robot.*
        # modules to remain importable/registered in sys.modules).
        removed = {}
...
class TestNoMLDependencyAtRuntime:
    def test_app_ml_not_imported_as_a_side_effect(self):
        # See TestNoRobotDependencyAtRuntime above for why removed
