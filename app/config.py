...
class MLConfig:
    """
    Settings for the ML intent classifier (app.ml).

    artifact_dir is relative to the app.ml package by default (see
    app.ml.artifacts), matching how the rest of the application avoids
    hard-coding machine-specific absolute paths.
    """

    artifact_dir: str = _env_str("LABGUIDE_ML_ARTIFACT_DIR", "")
    min_confidence: float = _env_float("LABGUIDE_ML_MIN_CONFIDENCE", 0.0)
