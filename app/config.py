...
class MLConfig:
    """
    Settings for the ML intent-classification service
    (app.ml.service.MLIntentService).
    """

    # Confidence threshold policy: predictions below this value are
    # reported as IntentType.UNKNOWN even though a label was predicted.
    # 0.30 is derived from the committed artifact's behavior on text
    # with no training-vocabulary overlap (~0.294 confidence, the
    # model's class-prior floor for this 4-class problem) -- see
    # tests/ml/test_service.py::TestLowConfidenceUnknownPolicy.
    min_confidence: float = _env_float("LABGUIDE_ML_MIN_CONFIDENCE", 0.30)

    # Optional override directory for the trained artifact. Empty/unset
    # (the default) falls back to the bundled app/ml/artifacts location
    # (app.ml.inference.inference.DEFAULT_ARTIFACT_PATH).
    artifact_dir: str = _env_str("LABGUIDE_ML_ARTIFACT_DIR", "")

    def __post_init__(self) -> None:
        if not (0.0 <= self.min_confidence <= 1.0):
            raise ConfigurationError(
                "MLConfig.min_confidence must be within [0.0, 1.0], got "
